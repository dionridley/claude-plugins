...
- **Question:**
  ...
  After each screen is built, a browser agent will click through the actual UI
  to catch interaction errors (broken form submissions, missing event handlers,
  wrong state transitions) that code review alone can miss.

  Recommended — but requires the Playwright MCP server to be configured in
  your Claude Code settings.
  ```
- **Header:** "E2E Testing (Recommended)"
- **Options:**
  1. "Yes, use Playwright MCP (Recommended)" — Description: "A browser agent will test each screen after it's built. Requires Playwright MCP in Claude settings."
...
# Install React Router and SQLite dependencies
npm install react-router-dom better-sqlite3 drizzle-orm
...
1. Read `[slug]/src/index.css` and prepend `@import "tailwindcss";`
2. Read `[slug]/vite.config.ts` and add the Tailwind plugin:
   ```typescript
...
**2. Create project (always use `--no-install`):**
```bash
...
**3. If `.tool-versions` was created in Phase 3**, move it into the project directory:
```bash
mv .tool-versions [slug]/.tool-versions
```
Or create it fresh in the project directory if it wasn't created earlier.
...
Read `[slug]/mix.exs` and add to the `deps` section under `only: [:dev, :test]`:
```elixir
{:faker, "~> 0.18", only: [:dev, :test]}
```
Then run `mix deps.get` again to fetch Faker.
...
**9. Set port in `config/dev.exs`:**
If port 4001 was chosen in Phase 3, read `config/dev.exs` and update the endpoint config:
```elixir
http: [ip: {127, 0, 0, 1}, port: 4001],
```

**10. Apply scaffold patches to `lib/[slug]_web/components/layouts/root.html.heex`:**
...
**11. Remove Phoenix boilerplate:**
```bash
rm -f lib/[slug]_web/controllers/page_controller.ex
rm -f lib/[slug]_web/controllers/page_html.ex
rm -rf lib/[slug]_web/controllers/page_html/
rm -f test/[slug]_web/controllers/page_controller_test.exs
```
...
**13. Verify dev server starts and assets compile:**
```bash
...
MVP project initialized!
  ...
  What was created:
    ...
    [x] [Stack] project scaffolded in ./[slug]
    [x] Dev server verified working (port [port])
    [x] Git repo initialized with initial commit

  E2E Testing: [Playwright MCP / disabled]

  Next steps:
    1. Review the brainstorm doc: .mvp/brainstorm.md
